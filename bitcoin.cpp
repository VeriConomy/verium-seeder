...
//    printf("%s: SEND %s\n", ToString(you).c_str(), pszCommand);
  ...
  void EndMessage() {
    ...
    if (vSend.GetVersion() >= FIRST_PROTOCOL_VERSION) {
  ...
  void PushVersion() {
    ...
    uint8_t fRelayTxs = 0;
    vSend << PROTOCOL_VERSION << nLocalServices << nTime << you << me << nLocalNonce << seeder_version << nBestHeight << fRelayTxs;
    EndMessage();
  }
...
//        printf("%s: added address %s (#%i)\n", ToString(you).c_str(), addr.ToString().c_str(), (int)(vAddr->size()));
  ...
  bool ProcessMessages() {
    ...
    do {
      ...
      if (vRecv.GetVersion() >= FIRST_PROTOCOL_VERSION) {
...
public:
  CNode(const CService& ip, vector<CAddress>* vAddrIn) : you(ip), nHeaderStart(-1), nMessageStart(-1), vAddr(vAddrIn), ban(0), doneAfter(0), nVersion(0) {
    ...
    if (time(NULL) > 1329696000) {
      vSend.SetVersion(FIRST_PROTOCOL_VERSION);
      vRecv.SetVersion(FIRST_PROTOCOL_VERSION);
    }
  }
  bool Run() {
    bool res = true;
