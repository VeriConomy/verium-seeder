...
{
    return testnet ? TESTNET_MIN_HEIGHT : MAINNET_MIN_HEIGHT;
}
...
public:
  ...
  bool IsGood() const {
    ...
    if (clientVersion && clientVersion < REQUIRE_PROTOCOL_VERSION) return false;
  ...
  int GetBanTime() const {
    if (IsGood()) return 0;
    if (clientVersion && clientVersion < REQUIRE_PROTOCOL_VERSION) { return 604800; }
